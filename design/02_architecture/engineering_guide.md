...
**深冻结序列化**：任务状态快照需要序列化 ReAct 的完整历史消息、当前黑板数据、执行版本号、挂起凭证等信息。建议使用 JSON 序列化并存储至 Redis（设置 TTL），同时在 MySQL 中保存元数据。

**WebSocket 连接管理**：BFF 需要维护用户 ID 与 WebSocket 连接的映射关系。当用户断线重连时，需要将离线期间的消息从 `mao_offline_inbox` 表中推送给用户，并在聊天流底部加盖视觉分割线。
