...
详细展示飞书机器人和 Web 工作站两种渠道的完整入站转换、卡片下发和回调处理流程。

![渠道适配层时序图](../05_diagrams/rendered/11_channel_adapter_sequence.png)

### 5.7 无状态 Worker 挂起与唤醒时序图

详细展示 Worker 实例 A 挂起后线程销毁，回调唤醒时由全新的 Worker 实例 B 从 StateDB 拉取快照恢复执行现场的全过程。

![无状态 Worker 挂起与唤醒时序图](../05_diagrams/rendered/12_stateless_worker_resume.png)
...
### 7.8 渠道类型 (ChannelType)

```java
public enum ChannelType {
    /** Web 端工作站，入站协议 HTTP POST + SSE，出站协议 SSE / WebSocket */
    WEB("WEB"),
    /** 飞书机器人，入站协议 Webhook HTTP POST，出站调用飞书 OpenAPI */
    FEISHU("FEISHU"),
    /** 钉钉机器人，入站协议 Webhook HTTP POST，出站调用钉钉 OpenAPI */
    DINGTALK("DINGTALK"),
    /** 企业微信机器人，入站协议 Webhook HTTP POST，出站调用企微 API */
    WECOM("WECOM");
