...
> **v9.1 更新**：架构图新增 L2 层渠道适配层 (Channel Adapter)，支持 Web 端和飞书机器人双渠道接入。L4 执行面标注为"无状态 Worker 池"，黑板和快照均外置持久化至 StateDB。
...
| L2 | 统一接入网关层 | 统一处理 HTTP/WS 请求及外部事件回调，**含多渠道适配层 (Channel Adapter)** | BFF API 网关、统一事件网关、**渠道适配器 (Web/Feishu)** |
...
**LUI + GUI 融合交互与多渠道适配**：接收自然语言意图并流式输出。当涉及参数确认、高危写入时，底层引擎输出标准卡片协议，由 L2 层的**渠道适配器 (Channel Adapter)** 动态翻译为目标渠道格式（如 Web 端的动态表单组件，或飞书机器人的交互式 Message Card）。卡片提交后即锁定为不可交互的灰色只读状态，由后端 State 绝对强控，不受浏览器刷新或跨设备登录影响。

**AI 托管大盘 (My Scheduler)**：侧边栏独立展示个人名下的后台定时任务和盯盘任务，支持直观查看存活期、一键暂停/恢复、强制终止以及过期续签。

**会话记忆温和管控与内外部记忆物理隔离**：为防 Token 爆仓，后端采用滑动窗口压缩记忆。为避免工作流产生的中间噪音污染 C 端用户的对话历史，系统严格区分**会话记忆 (Session Memory, `mao_message`)** 与**任务暂存区 (Task Scratchpad, `mao_task_log`)**。L4 执行面在推演过程中的 Thought、Action、Observation 绝不写入 `mao_message`，仅记录在 `mao_task_log` 中供审计使用。当且仅当任务彻底结束或需要人类介入时，才由 Worker 生成一段摘要文本或视图卡片写入 `mao_message`，以此保持用户会话窗口的高信噪比。当触发深度压缩时，前端聊天框顶部需轻量弹出提示，并强化顶部的【清空重置】按钮视觉。
...
**双引擎混合编排（含环路防护）**：ReAct 引擎与 DAG 画布引擎支持有限度的混合编排（Agent in Workflow, Workflow in Agent）。但为防止"双引擎无限嵌套"导致的死循环与堆栈溢出，**严格禁止 Agent 挂载包含其自身的宏工具**。在 Agent 发布和宏工具绑定时，系统必须执行 DFS 环路检测，一旦发现循环依赖（如 Agent A → Macro B → Agent A）即将阻断并返回错误码 `422 MACRO_CYCLE_DETECTED`。

#### 3.2.3 强类型共享黑板 (Typed Blackboard) 与无状态实例

在 DAG 节点流转中彻底废弃易导致幻觉的全量对话透传。所有 Agent 通过**外置持久化的全局哈希表（黑板）**进行数据握手。
...
| **视图交互** | `VIEW` | 阻断服务端推理流转，输出标准卡片协议交由渠道适配层下发（Web/飞书），执行轻量级挂起，Worker 线程销毁 | 任务配置确认单、风控参数填写 |
| **长程异步** | `ASYNC` | 声明式打上 `x_mao_suspend: true` 标签，触发大模型线程阻断与"状态机深冻结"，序列化落盘 Redis 等待外部回调唤醒 | OA 审批流、人工复核 |
| **复合编排** | `MACRO` | 将完整 DAG 打包为单一工具启动，移交引擎内部调度嵌套。**禁止包含当前 Agent 的环路调用**，发布时强制 DFS 环路检测 | 上线前资损风险排查 SOP、客诉补发补偿金 |
