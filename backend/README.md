...
### 多模型切换（Model Failover）

ReAct Runner 支持按 Agent 维度配置多模型候选，在主模型出现限流或短暂故障时自动切换到备用模型，降低任务中断概率。

`model_config_data` 示例：

```json
{
  "model": "gpt-4o",
  "model_fallbacks": ["gpt-4o-mini", "gpt-4.1-mini"],
  "temperature": 0.2,
  "max_steps": 7
}
```

也可直接提供 `model_candidates`（优先级高于 `model + model_fallbacks`）：
